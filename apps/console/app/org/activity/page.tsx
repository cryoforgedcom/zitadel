...
import { getActivityLogByOrganization } from "@/lib/mock-data"
...
export default function OrgActivityPage() {
  const { currentOrganization, currentInstance } = useAppContext()
  ...
  if (!currentOrganization) {
    return (
      <OrganizationSelectorPrompt 
        ...
        description="Choose an organization to view its activity log"
        targetPath="/org/activity"
      />
    )
  }
  
  const orgActivity = getActivityLogByOrganization(currentOrganization.id)

  // Apply filters
  const filteredActivity = useMemo(() => {
    return orgActivity.filter(entry => {
      // Time range filter
  ...
  }, [orgActivity, timeRange, customDateRange, actorFilter, actionTypeFilter, resourceTypeFilter, severityFilter])

  const hasActiveFilters = timeRange !== "30d" || actorFilter || actionTypeFilter !== "all" || resourceTypeFilter !== "all" || severityFilter !== "all"
  ...
  // Export function
  const handleExport = () => {
    const headers = ["Timestamp", "Action", "Action Type", "Resource Type", "Resource", "Resource ID", "Actor", "Severity"]
    ...
    a.href = url
    a.download = `activity-log-${currentOrganization.name}-${new Date().toISOString().split("T")[0]}.csv`
    a.click()
  ...
  const uniqueActors = [...new Set(orgActivity.map(e => e.actorName))]
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Recent activity in {currentOrganization?.name} ({filteredActivity.length} of {orgActivity.length} events)
          </p>
        </div>
        <div className="flex items-center gap-2">
