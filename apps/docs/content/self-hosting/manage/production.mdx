...
ZITADEL supports [**metrics**](/apis/observability/metrics), **tracing**, **logging**, and **profiling** through a unified [instrumentation configuration](/self-hosting/manage/instrumentation).

Enable Prometheus-compatible metrics scraping:
...
Enable distributed tracing via an OpenTelemetry collector:
...
See the [Instrumentation](/self-hosting/manage/instrumentation) page for the full configuration reference including all exporter types.
...
ZITADEL writes structured logs to standard error.
Configure the log level, output format, and which subsystems emit logs:

```yaml
Instrumentation:
  Log:
    Level: "INFO"
    Format: "json"       # "text", "json", "gcp", or "gcp_error_reporting"
    Streams:
      - runtime          # Startup, shutdown messages
      - ready            # Readiness and health checks
      - request          # Incoming API/HTTP requests
      - event_handler    # Event handling in projections
      - queue            # Job queue processing
```

Logs can also be exported to an OpenTelemetry backend in parallel:
...
For details on log fields and output structure, see [Log Structure](/self-hosting/manage/log-structure).
For the full logging configuration including masking and error options, see [Instrumentation](/self-hosting/manage/instrumentation).

### Why ZITADEL does not write logs to files

Log file management should not be in each business app's responsibility.
...
For example, systemd has journald and Kubernetes has Fluent Bit and Fluentd.
