...
// validPartitionName matches partition names generated by partitionName()
// (e.g. signals_2026030814 or signals_20260308). It is used to validate
// identifiers before interpolating them into DDL statements.
var validPartitionName = regexp.MustCompile(`^signals_\d{8,10}$`)
...
func (w *PartitionWorker) Work(ctx context.Context, _ *river.Job[PartitionArgs]) error {
	...
	for i := 0; i < 3; i++ {
		...
		// Evict rows from the default partition whose timestamps fall within
		// the new partition's range. Without this, CREATE TABLE ... PARTITION OF
		// fails with a constraint violation because PG cannot split rows out of
		// the default partition automatically.
		evict := fmt.Sprintf(
			`DELETE FROM signals.signals_default WHERE created_at >= '%s' AND created_at < '%s'`,
			startStr, endStr,
		)
		if res, err := w.db.ExecContext(ctx, evict); err != nil {
			logging.WithError(ctx, err).Warn("signal_partition.evict_default_failed",
				slog.String("partition", name),
			)
			// Continue — the CREATE may still succeed if there were no rows.
		} else if n, _ := res.RowsAffected(); n > 0 {
			logging.Info(ctx, "signal_partition.evicted_from_default",
				slog.String("partition", name),
				slog.Int64("rows", n),
			)
		}
...
func (w *PartitionWorker) dropExpiredPartitions(ctx context.Context, cutoff time.Time) error {
	...
	for _, name := range partitions {
		if !validPartitionName.MatchString(name) {
			logging.Warn(ctx, "signal_partition.invalid_name_skipped",
				slog.String("partition", name),
			)
			continue
		}
...
// queue. Must be called before q.Start(). No-op when worker is nil.
func RegisterPartitionWorker(ctx context.Context, q *queue.Queue, worker *PartitionWorker) {
	if worker == nil {
		return
	}
...
// called after q.Start(). No-op when worker is nil.
func StartPartitionSchedule(ctx context.Context, q *queue.Queue, worker *PartitionWorker) {
	if worker == nil {
		return
	}
