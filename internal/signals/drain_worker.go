...
func RegisterDrainWorker(ctx context.Context, q *queue.Queue, worker *DrainWorker) {
	if worker == nil {
		return
	}
...
func StartDrainSchedule(ctx context.Context, q *queue.Queue, worker *DrainWorker) {
	if worker == nil {
		return
	}
	interval := worker.cfg.DrainInterval
	if interval <= 0 {
