...
func NewPGStore(db *sql.DB, cfg SnapshotConfig) *PGStore {
...
func (s *PGStore) Save(ctx context.Context, signal Signal, findings []RecordedFinding, _ SnapshotConfig) error {
...
func (s *PGStore) Snapshot(ctx context.Context, signal Signal, cfg SnapshotConfig) (Snapshot, error) {
	...
	if signal.UserID != "" {
		signals, err := s.querySignals(ctx,
			`SELECT instance_id, created_at, caller_id, user_id, session_id, fingerprint_id,
			 ...
			 LIMIT $4`,
			signal.InstanceID, signal.UserID, cutoff, cfg.MaxSignalsPerUser,
		)
		if err != nil {
	...
	if signal.SessionID != "" {
		signals, err := s.querySignals(ctx,
			`SELECT instance_id, created_at, caller_id, user_id, session_id, fingerprint_id,
			 ...
			 ORDER BY created_at ASC
			 LIMIT $4`,
			signal.InstanceID, signal.SessionID, cutoff, cfg.MaxSignalsPerSession,
		)
		if err != nil {
...
func (s *PGStore) insertSignal(ctx context.Context, exec queryExecutor, signal Signal, findings []RecordedFinding) error {
	meta := signalMetadata{
	...
	if len(findings) > 0 {
		meta.Findings = append([]RecordedFinding(nil), findings...)
		meta.FindingNames = make([]string, len(findings))
		for i, finding := range findings {
			meta.FindingNames[i] = finding.Name
		}
	}
	...
	var err error
	hasMetadata := meta.AcceptLanguage != "" || len(meta.ForwardedChain) > 0 ||
		meta.Referer != "" || meta.SecFetchSite != "" || meta.IsHTTPS || len(meta.Findings) > 0
	if hasMetadata {
		metaJSON, err = json.Marshal(meta)
		if err != nil {
			return fmt.Errorf("signal store marshal metadata: %w", err)
	...
	for rows.Next() {
		...
		if metaJSON.Valid && metaJSON.String != "" {
			...
			if err := json.Unmarshal([]byte(metaJSON.String), &meta); err == nil {
				...
				rs.Findings = append([]RecordedFinding(nil), meta.Findings...)
				if len(rs.Findings) == 0 && len(meta.FindingNames) > 0 {
					rs.Findings = make([]RecordedFinding, len(meta.FindingNames))
					for i, name := range meta.FindingNames {
						rs.Findings[i] = RecordedFinding{Name: name}
					}
				}
			}
		}
