...
func compileRule(r Rule) (CompiledRule, error) {
	...
	if r.Engine == EngineRateLimit {
		...
		if r.RateLimitCfg.Window < time.Second {
			return CompiledRule{}, fmt.Errorf("rate_limit window must be at least 1s")
		}
		if r.RateLimitCfg.Max <= 0 {
			return CompiledRule{}, fmt.Errorf("rate_limit max must be greater than 0")
		}
	}
